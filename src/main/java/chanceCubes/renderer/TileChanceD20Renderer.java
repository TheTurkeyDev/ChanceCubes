...
public class TileChanceD20Renderer extends TileEntitySpecialRenderer<TileChanceD20>
{
	...
	@Override
	public void renderTileEntityAt(TileChanceD20 te, double x, double y, double z, float partialTicks, int var9)
	{
		GL11.glPushMatrix();

		GlStateManager.translate(x + 0.5F, y + 1.5F + te.wave, z + 2.5F);
		Tessellator tessellator = Tessellator.getInstance();
		float f1 = ((float) te.getWorld().getTotalWorldTime() % 750 + partialTicks) / 750.0F;

		BlockPos tp = te.getPos();
		random.setSeed((long) (tp.getX() * tp.getY() * tp.getZ()));
		...
		float color = ((te.getWorld().getTotalWorldTime() % 75) / 75F);
