...
public class ThermalExpansionModHook extends BaseModHook
{
	...
	public void loadRewards()
	{
		...
		if(!stack.isEmpty())
		{
			...
			GlobalCCRewardRegistry.DEFAULT.registerReward(new BasicReward(this.modId + ":florbs", 60, new MessageRewardType(new MessagePart("Florbs!!")), new ItemRewardType(new ItemPart(stack1))
			{
				@Override
		...
		if(!stack.isEmpty())
		{
			...
			GlobalCCRewardRegistry.DEFAULT.registerReward(new BasicReward(this.modId + ":stay_hydrated", 10, new MessageRewardType(new MessagePart("Remember to stay hydrated!")), new ItemRewardType(new ItemPart(stack))));
		}
	}
}
