...
public class ThermalExpansionModHook extends BaseModHook
{
	...
	public void loadRewards()
	{
		...
		if(!stack.isEmpty())
		{
			...
			GlobalCCRewardRegistry.INSTANCE.registerReward(new BasicReward(this.modId + ":Florbs", 60, new MessageRewardType(new MessagePart("Florbs!!")), new ItemRewardType(new ItemPart(stack1))
			{
				@Override
		...
		if(!stack.isEmpty())
		{
			...
			GlobalCCRewardRegistry.INSTANCE.registerReward(new BasicReward(this.modId + ":Stay_Hydrated", 10, new MessageRewardType(new MessagePart("Remember to stay hydrated!")), new ItemRewardType(new ItemPart(stack))));
		}
	}
}
