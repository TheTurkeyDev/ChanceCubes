...
public class HerobrineReward implements IChanceCubeReward
{
	...
	{
		update(world, pos, player, 0, rand.nextInt(5) == 1);
	}
	...
	private void update(final World world, final BlockPos pos, final EntityPlayer player, int stage, boolean staying)
	{
		switch(stage)
		{
			case 0:
			...
			{
				if(staying)
					RewardsUtil.sendMessageToAllPlayers(world, "<Herobrine> " + staySayings[rand.nextInt(staySayings.length)]);
				else
					RewardsUtil.sendMessageToAllPlayers(world, "<Herobrine> " + leaveSayings[rand.nextInt(leaveSayings.length)]);
				break;
			}
			case 2:
			{
